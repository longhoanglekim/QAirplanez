  ...
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-xl max-h-[90vh] overflow-y-auto
      ...
      <div class="grid grid-cols-6 gap-2 mb-4 overflow-y-auto max-h-32
        ...
        <div v-for="seat in firstSeats" :key="seat.id"
             @click="seat.available && handleSeatClick(seat)"
        class="rounded border-2 border-orange-500"
        :class="{
        'bg-gray-300 text-gray-500 cursor-not-allowed pointer-events-none': !seat.available,
        'bg-orange-500 text-white': currentSelectedSeats.includes(seat.id),
        'bg-white text-orange-500': seat.available && !currentSelectedSeats.includes(seat.id),
        'hover:bg-orange-600 hover:text-white': seat.available && !currentSelectedSeats.includes(seat.id)
        }">
        {{ seat.id }}
      </div>
      ...
      <!-- Hạng thương gia -->
      <div v-for="seat in businessSeats" :key="seat.id"
           @click="seat.available && handleSeatClick(seat)"
           class="rounded border-2 border-orange-500"
           :class="{
         'bg-gray-300 text-gray-500 cursor-not-allowed pointer-events-none': !seat.available,
      ...
      <div v-for="seat in economySeats" :key="seat.id"
           @click="seat.available && handleSeatClick(seat)"
           class="rounded border-2 border-orange-500"
           :class="{
         'bg-gray-300 text-gray-500 cursor-not-allowed pointer-events-none': !seat.available,
    ...
    <div class="p-6 border-t flex justify-between items-center bg-amber-50">
      ...
      <div class="space-x-4">
        <button class="px-4 py-2 bg-gray-100 text-gray-800 rounded border border-gray-300 hover:bg-gray-200"
                @click="$emit('close')">
          Hủy
        </button>
