...
<div class="max-w-4xl mx-auto p-4 bg-gradient-to-br from-orange-50 to-red-50">
    <header class="flex items-center justify-center mt-20 mb-5">
    ...
    <div class="bg-white shadow-md rounded-lg mb-4 border border-red-100" >
        <div class="p-4 bg-red-50 border-b border-red-100 flex justify-between items-center cursor-pointer"
        ...
        <div class="mt-4">
            <p>Ghế đã chọn: {{ selectedSeat.join(', ') }}</p>
        </div>
    </div>
    ...
    <div class="bg-red-100 p-4 rounded-lg text-center shadow-md">
        ...
        <button class="mt-4 w-1/2 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors duration-300">
            Xác nhận dịch vụ
        </button>
    </div>
        ...
        <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 max-h-[80vh] overflow-y-auto" @click.stop>
            ...
            <div v-if="activeModal === 'seat'" class="p-6">
                ...
                <div class="grid grid-cols-6 gap-2">
                    <button v-for="(seat, index) in seatMap" :key="index" @click="toggleSeat(seat)" class="py-2 border rounded-lg" 
                    :class="selectedSeat.includes(seat)
...
<script setup>
import BookingProgressBar from '@/components/composable/BookingProgressBar.vue';</script>
...
export default {
    ...
    methods: {
        ...
        toggleSeat(seat) {
            ...
            if (index > -1) {
                // Seat already selected, so remove it
                this.selectedSeat.splice(index, 1);
            } else {
                // Seat not selected, so add it
